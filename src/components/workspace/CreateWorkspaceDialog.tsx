...
export function CreateWorkspaceDialog({
  ...
  useEffect(() => {
    ...
    getDefaultBranch(selectedRepoPath)
      .then((b) => {
        setDefaultBranch(b);
        setBaseBranch(b);
      })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!name.trim() || !selectedRepoPath) return;
    ...
    try {
      ...
      await onSubmit(selectedRepoPath, name.trim(), branchName, base);
      onClose();
    } catch (err) {
      setError(String(err));
    } finally {
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-3">
        ...
        <div>
          ...
          <p className="text-[11px] text-zinc-600 mt-1">
            Creates a new git worktree and branch for this workspace
          </p>
        </div>
          ...
          <div className="flex flex-col gap-3 pl-2 border-l-2 border-zinc-800">
            ...
            <div>
              ...
              <input
                ...
                placeholder={name || "Same as workspace name"}
                className="w-full h-8 px-3 text-sm rounded-md bg-zinc-800 border border-zinc-700 text-zinc-100 placeholder:text-zinc-500 focus:outline-none focus:border-indigo-500"
              />
            </div>
        ...
        <div className="flex justify-end gap-2 pt-1">
          ...
          <Button
            type="submit"
            size="sm"
            disabled={!name.trim() || !selectedRepoPath || loading}
          >
